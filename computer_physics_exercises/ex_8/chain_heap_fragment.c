...
void heap_insert(global_t *glob, event_t *event, int ev)
{
  ...
  glob->heap[pos].t = event[ev].t;       /* insert event */
  glob->heap[pos].event = ev;          /* insert event */
  event[ev].pos = pos;                 /* store position */

  while(pos > 0 )          /* move up in heap until */
  {                      /* father has lower time */
    father = (pos-1)/2;
    if(glob->heap[father].t <= glob->heap[pos].t)
      break;
    elem = glob->heap[father];           /* exchange */
    glob->heap[father] = glob->heap[pos];
    glob->heap[pos] = elem;
    event[glob->heap[pos].event].pos = pos; /* store pos */
    pos = father;                          /* move up */
  }
  event[glob->heap[pos].event].pos = pos; /* store pos */
}
...
void heap_remove(global_t *glob, event_t *event, int pos)
{
  ...
  else
  {
	...
	if(glob->heap[left].t < glob->heap[right].t)
      ...
      {
	finished = 1;     /* minimum son does NOT have lower time */
	break;
      }
...
particle_t *init_particles(global_t *glob)
{
  ...
  for(p=1; p<=glob->n; p++)          /* other particles */
  {
    ...
    // part[p].m = randomNumber;
    

    // if (k == 0) {           /* alternating masses */
    //   part[p].m = 1.1;
    //   k = 1;
    // } else {
    //   part[p].m = 1;
...
/* main to test heap_instert function */
...
//   // make a random heap which satifies the heap property of having the smallest element at the top and the largest at the bottom of the heap
//   glob.n = 5;
//   glob.heap = (heap_elem_t *) 
//     malloc((glob.n+1)*sizeof(heap_elem_t));
//   glob.heap_num = glob.n;
//   event = (event_t *) malloc( (glob.n+1)*sizeof(event_t));
//   glob->heap[right].t = 15,
//   event[0].t = 15;
//   event[1].t = 17;
//   event[2].t = 19;
//   event[3].t = 21;
//   event[4].t = 23;
//   event[5].t = 25;
//   int pos = glob.heap_num++;

//   // show the heap before insert
//   printf("Before insert\n");
//   for(ev=0; ev<=glob.n; ev++)
//     printf("%d [%d,%d] %f\n", ev, event[ev].pl, 
//     event[ev].pr, event[ev].t);

//   // insert only 13 value into the heap
//   heap_insert(&glob, event, 20);
  
//   // show the heap after insert
//   printf("After insert\n");
//   for(ev=0; ev<=glob.n; ev++)
//     printf("%d [%d,%d] %f\n", ev, event[ev].pl, 
//     event[ev].pr, event[ev].t);
...
//     printf("%d [%d,%d] %f\n", ev, event[ev].pl, 
//     event[ev].pr, event[ev].t);*/
//   // Call heap_insert function here to test it
//   // heap_insert(&glob, event, 0);
